...
def per_example_scores(predicted_objective: np.ndarray, predicted_constraints: np.ndarray,
    ...
    if fp > 0 or fn > 0:
        print('---')
        print('Predicted obj:')
        print(predicted_objective)
        print('True obj:')
        print(true_objective)
        print('Predicted constraints:')
        print(predicted_constraints)
...
def overall_score(predicted_objectives: List[np.ndarray], predicted_constraints: List[np.ndarray],
    ...
    for i, (p_obj, p_const, t_obj, t_const) in enumerate(zip(predicted_objectives, predicted_constraints,
        ...
        if len(p_obj) != len(t_obj):
            wrong_num_var += 1
        if p_const.shape[0] > t_const.shape[0]:
            too_many_const += 1
        if p_const.shape[0] < t_const.shape[0]:
            too_few_const += 1
        if fp > 0 or fn >0:
    ...
    print(f"Wrong num variables: {wrong_num_var/len(predicted_constraints)}")
    print(f"Too many constraints: {too_many_const/len(predicted_constraints)}")
    print(f"Too few constraints: {too_few_const/len(predicted_constraints)}")
    print(f"Something wrong: {something_wrong/len(predicted_constraints)}")

...
def convert_to_canonical_two_sided(formulation: ProblemFormulation):
    n_entities = max(formulation.entities.values()) + 1 if len(formulation.entities) else 0
    constraints = []
    objective = np.zeros((n_entities + 1 )* 2)
    ...
    for constraint in formulation.constraints:
        ...
        row_lhs = np.ones(n_entities + 1)
        row_rhs = np.ones(n_entities + 1)

        # compute everything as <= at first
        if constraint.type == const.SUM_CONSTRAINT:
            # x + y <= 150
            row_rhs *= 0
            row_rhs[-1] = constraint.limit
            ...
            for k, v in constraint.terms.items():
                # check if value was given in formulation
                row_lhs[v.index] = v.value if v.value is not None else np.nan
            row_rhs[-1] = constraint.limit
        elif constraint.type == const.RATIO_CONTROL_CONSTRAINT:
            # x <= 0.7 (x + y)
            row_rhs *= constraint.limit
            row_lhs *= 0
            for k, v in constraint.terms.items():
                row_lhs[v.index] = 1
            row_lhs[-1] = 0
        ...
        if constraint.operator == const.GT:
            row_lhs, row_rhs = row_rhs, row_lhs
        constraints.append(np.concatenate((row_lhs,row_rhs)))

    return objective, constraints

def convert_to_canonical_two_sided_wordform(formulation: ProblemFormulation):
    n_entities = max(formulation.entities.values()) + 1 if len(formulation.entities) else 0
    row_size = n_entities + 1
    constraints = []
    objective = [0] * (row_size * 2)
    for k, v in formulation.objective.terms.items():
        # check if value was given in formulation
        # if v.text_value == '':
        #     print('')
        objective[v.index] = v.text_value if v.text_value is not None else np.nan
    ...
    for constraint in formulation.constraints:
        ...
        if constraint.type == const.SUM_CONSTRAINT:
            ...
            row_lhs[-1] = 0
            row_rhs = [0] * row_size
            row_rhs[-1] = constraint.text_limit
            ...
            for k, v in constraint.terms.items():
                ...
                # if v.text_value == '':
                #     print('')
                row_lhs[v.index] = v.text_value if v.text_value is not None else np.nan
            row_rhs[-1] = constraint.text_limit
        elif constraint.type == const.RATIO_CONTROL_CONSTRAINT:
            # x <= 0.7 (x + y)
            row_rhs = [constraint.text_limit] * row_size
            row_lhs = [0] * row_size
            for k, v in constraint.terms.items():
                row_lhs[v.index] = 1
            row_lhs[-1] = 0
            ...
            # y should be first term, but we will parse whichever term that has value as y
            for i, (k, v) in enumerate(constraint.terms.items()):
                if v.text_value is not None and v.text_value != '':
                    row_rhs[v.index] = v.text_value
                else:
                    row_lhs[v.index] = 1
        elif constraint.type == const.BALANCE_CONSTRAINT_2:
            # x <= y
        ...
        if constraint.operator == const.GT:
            row_lhs, row_rhs = row_rhs, row_lhs
        constraints.append(row_lhs + row_rhs)
...
class JSONFormulationParser(Parser):
    ...
    def parse_objective(self, data: dict, vars: dict, order_mapping: dict) -> ObjectiveDeclaration:
        ...
        if 'terms' in data:
            for i, (k, v) in enumerate(data['terms'].items()):
                terms[k] = Term(name=k, index=order_mapping[k], value=self.parse_number(v), text_value=v)
        else:
            # assume values are 1 if there is no terms mapping
            for var in vars:
                terms[var] = Term(name=var, index=order_mapping[var], value=1, text_value='1')
    ...
    def parse_constraint(self, data: dict, entities: dict, var_dict: dict) -> ConstraintDeclaration:
        ...
        text_limit = data['limit'] if 'limit' in data else ""
        ...
        if 'terms' in data:
            for k, v in data['terms'].items():
                terms[k] = Term(name=k, index=entities[k], value=self.parse_number(v), text_value=v)
            ...
            k = data['y_var']
            terms[k] = Term(name=k, index=entities[k], value=self.parse_number(data['param']), text_value=data['param'])
            k = data['x_var']
            terms[k] = Term(name=k, index=entities[k])
